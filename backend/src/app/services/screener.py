...
Lazy-imports VectorBT (heavy dependency) only when actually running screens.
"""
...
class ScreenerService:
    ...
    def run_full_screen(params: dict, progress_cb: Callable) -> dict:
        """Run all 4 strategy screens (sync, long-running — for job runner)."""
    ...
    async def list_results(self, limit: int = 20) -> list[ScreenResult]:
        stmt = (
            select(ScreenResult)
    ...
    async def get_result(self, result_id: int) -> ScreenResult | None:
        result = await self._session.execute(
            select(ScreenResult).where(ScreenResult.id == result_id)
        )
        return result.scalar_one_or_none()

    async def save_results(self, job_id: str, screen_output: dict) -> list[ScreenResult]:
        """Persist screen results from a completed job."""
        symbol = screen_output.get("symbol", "")
        timeframe = screen_output.get("timeframe", "")
        strategies = screen_output.get("strategies", {})
        saved = []
        ...
        for os_lvl in [20, 25, 30, 35]:
            for ob_lvl in [65, 70, 75, 80]:
                ...
                try:
                    ...
                    results.append({
                        "rsi_period": period, "oversold": os_lvl, "overbought": ob_lvl,
                        "total_return": pf.total_return(), "sharpe_ratio": pf.sharpe_ratio(),
                        "max_drawdown": pf.max_drawdown(),
