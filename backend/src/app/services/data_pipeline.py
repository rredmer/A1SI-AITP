...
from app.services.platform_bridge import ensure_platform_imports, get_processed_dir
...
class DataPipelineService:
    ...
    def list_available_data(self) -> list[dict]:
        """List all Parquet data files with metadata (sync, fast)."""
        import pandas as pd
    ...
    def get_data_info(self, symbol: str, timeframe: str, exchange: str) -> dict | None:
        """Get detailed info about a specific data file (sync, fast)."""
        import pandas as pd
    ...
    def download_data(params: dict, progress_cb: Callable) -> dict:
        """Download OHLCV data from exchange (sync, long-running — for job runner)."""
        ensure_platform_imports()
    ...
    def generate_sample_data(params: dict, progress_cb: Callable) -> dict:
        """Generate synthetic OHLCV data for testing (sync, long-running)."""
        import numpy as np
        import pandas as pd
        ...
        for symbol in symbols:
            base_price = {"BTC/USDT": 50000, "ETH/USDT": 3000, "SOL/USDT": 100}.get(
                symbol, 100
            )
            for tf in timeframes:
                done += 1
